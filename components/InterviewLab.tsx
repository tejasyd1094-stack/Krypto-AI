...
interface InterviewLabProps {
  /**
   * Added userCredits to fix scope error where it was being accessed in handleStartSession
   */
  userCredits: number;
  userLocation?: string;
        ...
        <form onSubmit={handleStartSession} className="space-y-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
            <div className="space-y-3">
              ...
              <input required value={inputs.company} onChange={e => setInputs({...inputs, company: e.target.value})} className="w-full bg-zinc-950 border border-zinc-900 rounded-2xl px-6 py-5 text-sm outline-none focus:border-yellow-500/50 uppercase font-bold text-zinc-100" placeholder="e.g. Google" />
            </div>
            <div className="space-y-3">
              <label className="text-[10px] font-black text-zinc-600 uppercase tracking-widest px-1">Target Role</label>
              <input required value={inputs.role} onChange={e => setInputs({...inputs, role: e.target.value})} className="w-full bg-zinc-950 border border-zinc-900 rounded-2xl px-6 py-5 text-sm outline-none focus:border-yellow-500/50 uppercase font-bold text-zinc-100" placeholder="e.g. Cloud Engineer" />
            </div>
            <div className="space-y-3 sm:col-span-2">
              <label className="text-[10px] font-black text-zinc-600 uppercase tracking-widest px-1">Target Geography (Optional)</label>
              <div className="relative">
                 <input 
                   value={inputs.location} 
                   onChange={e => setInputs({...inputs, location: e.target.value})} 
                   className="w-full bg-zinc-950 border border-zinc-900 rounded-2xl px-6 py-5 text-sm outline-none focus:border-yellow-500/50 uppercase font-bold pr-12 text-zinc-100" 
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-4">
              {['behavioral', 'technical', 'cultural'].map(t => (
                <button key={t} type="button" onClick={() => setInputs({...inputs, type: t as any})} className={`px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${inputs.type === t ? 'bg-yellow-500 text-zinc-950 shadow-xl' : 'bg-zinc-900 text-zinc-500 border border-zinc-800'}`}>
                  {t} SESSION
                </button>
              ))}
            </div>
      ...
      {result && (
        <div ref={resultRef} className="bg-[#0c0c0e] border border-blue-500/20 rounded-[56px] p-10 sm:p-16 animate-in slide-in-from-bottom-8 scroll-mt-24 text-zinc-100">
          <div className="flex items-center justify-between mb-10">
            ...
            {inputs.location && (
              <span className="text-[10px] font-black text-blue-500/60 uppercase tracking-widest uppercase text-zinc-100">Localized for {inputs.location}</span>
            )}
          </div>
